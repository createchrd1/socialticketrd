...
function Bandeja() {
  ...
  const active = items.find((c) => c.id === activeId) ?? visible[0] ?? items[0]!;
            ...
            <div className="ml-auto flex items-center gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {team.map((p) => (
                    <DropdownMenuItem key={p} onClick={() => update(active.id, { assignee: p })}>
                      {p}
                    </DropdownMenuItem>
                  ))}
                  <DropdownMenuItem onClick={() => update(active.id, { assignee: null })}>
                    Sin asignar
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
